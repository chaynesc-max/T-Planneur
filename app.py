# This cell contains the entire Streamlit application code.
# Running this cell will start the Streamlit server.
# Access the application through the public URL provided by pyngrok after execution.
...
# ------------------------------
st.sidebar.subheader("Saisir les congés validés des employés")
conges_input = {}
for i in range(nb_employes):
   emp = f"E{i+1}"
   conges_input[emp] = st.sidebar.date_input(
       label=f"Congés pour l'employé {emp} (Ctrl+clic pour plusieurs dates)",
       value=[],
...
def build_or_tools_model(nb_employes, num_days, shift_types, shift_index, jours, conges_input):
   ...
   # Shift hours (scaled for integer arithmetic)
   scale_factor = 4
   shift_hours_scaled = {
   ...
   for e_idx in range(nb_employes):
      for d_idx in range(num_days):
          ...
          if d_idx < num_days - 1: # Ensure d_idx+1 is within bounds
               # Create boolean variable for the condition on the next day being REPOS
               next_day_is_repos = model.NewBoolVar(f'e{e_idx}_d{d_idx+1}_is_repos')
               model.Add(planning_vars[(e_idx, d_idx + 1)] == shift_index[REPOS]).OnlyEnforceIf(next_day_is_repos)
               model.Add(planning_vars[(e_idx, d_idx + 1)] != shift_index[REPOS]).OnlyEnforceIf(next_day_is_repos.Not())

               # Add the implication using the boolean variables
               model.AddImplication(shift_is[NUIT_SEMAINE], next_day_is_repos)
               model.AddImplication(shift_is[WEEKEND_NUIT], next_day_is_repos)
          ...
          for week_start_idx in range(0, num_days, 7):
              ...
              for day_in_week_idx in range(week_start_idx, week_end_idx - 1): # Need at least 2 days remaining
                  ...
                  model.AddBoolAnd([
                      planning_vars[(e_idx, day_in_week_idx)] == shift_index[REPOS],
                      planning_vars[(e_idx, day_in_week_idx + 1)] == shift_index[REPOS]
                  ]).OnlyEnforceIf(is_two_day_repos)
          ...
          if weekday == 4 and d_idx < num_days - 2: # Friday and enough days for Sat (d_idx+1) and Sun (d_idx+2)
               ...
               model.AddImplication(shift_is[NUIT_SEMAINE], is_next_day_weekend_nuit)
               model.AddImplication(shift_is[NUIT_SEMAINE], is_day_after_next_weekend_nuit)
